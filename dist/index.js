...
async function callApi(endpoint, payload, method = 'POST') {
    ...
    if (payload)
        opts.body = JSON.stringify({ key: API_KEY, ...payload });
    try {
...
// ---- memory/search ----
server.tool('memory_search', 'Search persistent memory for relevant facts. Returns facts + token savings.', { query: z.string().max(500).describe('What to search for'), limit: z.number().min(1).max(20).optional().default(5) }, async ({ query, limit }) => {
    const res = await callApi('/memory/search', { query, limit });
    if (!res.ok)
        return { content: [{ type: 'text', text: `Error: ${res.message || 'unknown'}` }] };
    if (!res.facts?.length)
        return { content: [{ type: 'text', text: 'No facts found. Add some with memory_add.' }] };
    const lines = res.facts.map((f) => `• [${f.level}] ${f.label}: ${f.content}`);
...
server.tool('web_search', 'Fetch a web page and compress it through ViBo (saves 96-99% tokens).', { url: z.string().max(2048).describe('URL to fetch'), query: z.string().max(500).optional().describe('What you are looking for') }, async ({ url, query }) => ({
    content: [{ type: 'text', text: 'web_search is disabled in this build — it ships in the next release. Use your own fetch for now.' }],
}));
// ---- memory/add ----
server.tool('memory_add', 'Save a fact to persistent memory. Deduplicated by exact match.', { label: z.string().max(200).describe('Short name of the fact'), content: z.string().max(5000).describe('The fact to remember') }, async ({ label, content }) => {
...
// ---- memory/usage ----
server.tool('memory_usage', 'Get token savings statistics (today + all-time).', {}, async () => {
    if (!API_KEY) {
...
// ---- thread ----
server.tool('thread_memory', 'Thread memory: add message, compress old ones, ask details, get context.', {
    ...
    if (action === 'ask') {
        if (!res.results?.length)
            return { content: [{ type: 'text', text: 'Nothing found in thread history.' }] };
        return { content: [{ type: 'text', text: res.results.map((r) => `• ${r.content}`).join('\n') }] };
    }
    if (action === 'context') {
